...
def utils_check():
    print("Radar utils import success")
...
def open_device():
    ...
    if m_dev == INVALID_DEVICE_HANDLE:
        print("Open Device failed!, check whether the .so file is chosen appropriately(Differnt files for linux, Raspberry-pi, and Jetson)")
        exit(0)
...
def receive_can_data(dev_ch2):
    ...
    if ret > 0:
        ...
        for i in range(num):
            # print(f"[{i}]:ts:{rcv_can_msgs[i].timestamp}, id:0x{rcv_can_msgs[i].frame.can_id:X}, len:{rcv_can_msgs[i].frame.can_dlc}, "
            #     f"eff:{rcv_can_msgs[i].frame.eff}, rtr:{rcv_can_msgs[i].frame.rtr}, "
            #     f"data:{' '.join(f'{rcv_can_msgs[i].frame.data[j]:02X}' for j in range(rcv_can_msgs[i].frame.can_dlc))}")
            
            frame = {
                "timestamp": rcv_can_msgs[i].timestamp,
                "can_id": f"0x{rcv_can_msgs[i].frame.can_id:X}",
                "dlc": rcv_can_msgs[i].frame.can_dlc,
                "eff": rcv_can_msgs[i].frame.eff,
                "rtr": rcv_can_msgs[i].frame.rtr,
                "data": [f"{rcv_can_msgs[i].frame.data[j]:02X}" for j in range(rcv_can_msgs[i].frame.can_dlc)]
            }
            raw_msgs.append(frame)
