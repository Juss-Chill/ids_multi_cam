        ...
        for (size_t i = 0; i < deviceManager.Devices().size(); ++i)
        {
            if (deviceManager.Devices().at(i)->IsOpenable())
            {
                ...
                if (device)
                {
                    ...
                    deviceElem->nodemapRemoteDevice->FindNode<peak::core::nodes::IntegerNode>("BinningHorizontal")->SetValue(1); // divide the inbult resolution of 1920 X 1200 by this value of 4 => 480 X 300
                    ...
                    deviceElem->nodemapRemoteDevice->FindNode<peak::core::nodes::IntegerNode>("BinningVertical")->SetValue(1);

                    try
                    {
                        // Set auto exposure continuous
                        deviceElem->nodemapRemoteDevice->FindNode<peak::core::nodes::EnumerationNode>("ExposureAuto")->SetCurrentEntry("Continuous");
                    }
                    ...
                    try
                    {
                        ...
                        std::string value = deviceElem->nodemapRemoteDevice->FindNode<peak::core::nodes::EnumerationNode>("BalanceWhiteAuto")->CurrentEntry()->SymbolicValue();
                        // Get a list of all available entries of BalanceWhiteAuto
                        auto allEntries = deviceElem->nodemapRemoteDevice->FindNode<peak::core::nodes::EnumerationNode>("BalanceWhiteAuto")->Entries();
                        ...
                        deviceElem->nodemapRemoteDevice->FindNode<peak::core::nodes::EnumerationNode>("BalanceWhiteAuto")->SetCurrentEntry("Continuous");
                    ...
                    try
                    {
                        ...
                        std::string value = deviceElem->nodemapRemoteDevice->FindNode<peak::core::nodes::EnumerationNode>("GainAuto")->CurrentEntry()->SymbolicValue();
                        // Get a list of all available entries of GainAuto
                        auto allEntries = deviceElem->nodemapRemoteDevice->FindNode<peak::core::nodes::EnumerationNode>("GainAuto")->Entries();
                        std::vector<std::shared_ptr<peak::core::nodes::EnumerationEntryNode>> availableEntries;
                        for(const auto & entry : allEntries)
                        {
                            if ((entry->AccessStatus()!=peak::core::nodes::NodeAccessStatus::NotAvailable)
                        ...
                        // Set GainAuto to "Off"
                        deviceElem->nodemapRemoteDevice->FindNode<peak::core::nodes::EnumerationNode>("GainAuto")->SetCurrentEntry("Continuous");
